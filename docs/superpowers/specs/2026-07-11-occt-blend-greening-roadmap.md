...
| G3 | **Generic invalid-solid (triage)** | **DONE — reassigned** | — | **Triage complete 2026-07-12** (`docs/superpowers/specs/2026-07-12-g3-triage-reassignment.md`): full 217-FailFaulty sweep classified by `Health().Reason`. Reassigned → **G4 miter/corner-planar (93)**, **G5 n-way/mixed/arc-end (35)**, **G6 curved-neighbour (34)**, **G7 edge≠2 (5)**, and **26 REAL BUILD BUGS** — 22 "inconsistent orientation" (`simple/{K6,K9,L1,L3,L4,L6,L7,R9,S1,S3,S4,S6,S7,S9,T1,T3,T4,T7,T9,U4,X3,Y1}`, all build a body then fail validity on face winding — likely ONE root cause, "one fix unlocks many") + 4 empty-reason (`simple/{F6,T6,U3}`, `tolblend/D5`). Greened since original list: `A6,A8,K7,W1` (G1 fix), `V1` (runout setback), `X9`. **NEXT recommended fix = the 22 inconsistent-orientation build bugs** (focused face-winding root-cause hunt, not missing-feature IP) |
