...
**TASK-1 FINDING (localized 2026-07-14, DRAWEXE per-face surface-TYPE dump — supersedes an earlier area-only mis-assignment) — FILLET AXIAL-TRIM + FLAT-PATCH FILL:**
- Surface types via full-face `dump` (`whatis`/`dumpsurface` are uninformative in DRAWEXE 8.0.0; `mksurface`+`dump` prints the class). OCCT's 15 S1 faces: 2 boss walls (`CylindricalSurface` 565.487, 753.982), **2 fillet cylinders** (`CylindricalSurface` r6 axis-x: result_6=28.951, result_10=28.951 = **57.9**), **3 FLAT patches** (`BSplineSurface` whose control points are COPLANAR — result_3 all z=10; result_8/result_11 all y=−10; circular-arc boundaries force the b-spline encoding: 34.19+26.595+26.595 = **87.4**), and the box faces/caps (`Plane`, byte-identical to ours).
- **Fillet: 188.495 → 57.9 (Δ −130.6).** OCCT keeps the fillet cylinder ONLY on the boss-free outer axial segments `x∈[−10,−6.93]∪[6.93,10]` (each Δx≈3.07, full quarter cross-section 28.951). The whole interfered middle span `|x|<6.93` has **NO rounded fillet** — the bosses consume the convex edge there.
- **Host planes: 305.97 → 386.8 (Δ +80.8).** The freed middle span is filled with FLAT area in the host planes (z=10 and y=−10), bounded by the boss footprints (circles x²+y²=64 and x²+z²=36) and the fillet's former tangent rails. OCCT encodes these flat regions as coplanar b-splines; we may build them as planar `filletFace`s.
- Net: −130.6 + 80.8 = −49.8 = the surplus removed. ✓
- **Crossings** (still the trim/merge split points): r8 top boss ∩ tangent-rail y=−4 at **x=±6.93** (√48); r6 front boss ∩ tangent-rail z=4 at **x=±4.47** (√20).
- **Implication for Task 4/5 (revised):** the fix is (a) **trim the fillet to boss-free axial segments** — an axial-SPAN removal (drop the interfered middle), NOT a cross-section strip; and (b) **reconstruct the host planes over the freed span** as flat area bounded by the boss footprint arcs + the fillet-segment end rails. NO curved corner-blend surface is needed (the new patches are planar) — the plan's "no curved blend faces" architecture holds. Watertightness: the trimmed fillet segment ends must weld to the flat host-plane patches along shared rails; the boss walls/caps stay untouched.
