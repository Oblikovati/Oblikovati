...
| M11 | Assembly & Instancing | ✅ | All 8 features done (V1) → `model/occurrence` (placement, replication, patterns, events), `model/compdef` (`AssemblyComponentDefinition`, shrinkwrap/derive, assembly machining features + serialization), `model/proxy`, `model/bom` (+ `model/bomapi`) → `app` (occurrence/feature browser actions, sketch-in-assembly, extrude/revolve/hole/chamfer/fillet/move-face/box-cut/proxy-cut tools, BOM panel) → **head UI** (browser nodes, ribbons, ToolParams dialogs, BOM window + CSV export) + `addin/router` over `api/wire`. Occurrence-relative edge/face resolution (#735); feature program persists + undoes (#785). Two issues remain open, both Version-2. See the M11 section below. |
...
Tracked on GitHub (issues #345–#349, #355, #361, #632, #633, #718, #725, #730, #735, #762–#789,
plus #810 and #816); the "PBI" column carries the `M11-Fxx` feature id used throughout the code comments.

| PBI | Title | Status | Go package | Notes |
|-----|-------|:------:|------------|-------|
| F00 | Assembly document type | ✅ | `model/doc`, `cmd` | `AssemblyDocument` + `*AssemblyComponentDefinition` content (the M03/M11 document specialization), creatable from the CLI/head; stamps its own format (ADR-0034). |
| F01 | Assembly definition & occurrences | ✅ | `model/compdef`, `model/occurrence` | `AssemblyComponentDefinition` (the reference platform's AssemblyComponentDefinition, #345) owning a `ComponentOccurrences` collection that mints occurrence ids and tracks placements. |
| F02 | Component placement | ✅ | `model/occurrence`, `model/compdef` | Place a component document as an occurrence with a position transform (#346); real child geometry instanced through the occurrence (the flyweight — one definition, many placements). |
| F03 | Proxies | ✅ | `model/proxy` | Occurrence-scoped proxies that assembly features, mates and measurement resolve to a placement's geometry (#347). |
| F04 | Replication & patterns | ✅ | `model/occurrence` | Mirror / copy / substitute (`ops.go`, PBI-122) and rectangular/circular/feature-based occurrence patterns (`arrangement.go`/`pattern.go`, PBI-121); component-name propagation so replicas persist + undo (#765). |
| F05 | Bill of Materials | ✅ | `model/bom`, `model/bomapi` | Structured + parts-only views, quantity counting, BOM-structure classification, CSV export with iProperty-driven custom columns (#349, #730); served through `addin/router` and the head BOM window (#768). |
| F06 | Shrinkwrap & derived assembly | ✅ | `model/compdef` | `assembly_derive` flattens the occurrence tree into placed bodies; `shrinkwrap_lod` simplifies an assembly into a non-parametric base part placeable as a substitute (#348/#355/#361). |
| F07 | Assembly events | ✅ | `model/compdef`, `model/occurrence` | `AssemblyEvents` bridges raw occurrence mutations (placement/move/suppress/replace) into the reference platform's AssemblyEvents domain notifications, batched, with a no-op-state guard (#632). |
| F08 | Assembly machining features | ✅ | `model/compdef`, `model/feature` | Features authored in the assembly that cut/modify placed component geometry in place: extrude, revolve, hole, chamfer, fillet, move-face, box-cut, proxy-cut — each recomputed per participant via occurrence-relative edge/face resolution (#633/#725/#735). The feature program serializes + undoes as a tagged union (#785); in-session parameter edit (#816). |
