...
| `model/exchange/translators/olecf` | no — own `go.mod`, not in `go.work` | `make test-nested-modules` |
| `model/exchange/translators/inventor` | no — own `go.mod`, not in `go.work` | `make test-nested-modules` |
| `model/exchange/translators/solidworks` | no — own `go.mod`, not in `go.work` | `make test-nested-modules` |
...
**That table is generated by nobody, so nothing may depend on it.** The gate DERIVES its
module set from `git ls-files '*go.mod'` (`GATE_NESTED_MODULES` in the Makefile); a module
added tomorrow is in the gate the day it lands, with no edit. A hand-kept list would
re-introduce exactly the defect the gate exists to remove — silent under-coverage — on a
timer, so three guards in `archguard/gate_module_coverage_test.go` hold it:

- `TestGateReachesEveryTrackedModule` — every tracked `go.mod` is run by a named leg, or
  the test fails naming the path;
- `TestCIRunsTheSameNestedModulesAsTheGate` — `ci.yml`'s hand-typed coverage loop and the
  derived set agree;
- `TestCIWiresTheAPIContractIntoEveryModuleThatNeedsIt` — every nested module that
  requires `oblikovati.org/api` is in `ci.yml`'s api-contract input. (`olecf` is
  standard-library only and correctly absent from it — the shorter list is deliberate.)

`make gate` runs, in order: `head-deps`, `spdx-check`, `docs-lint`, `test-head`,
`test-nested-modules`, `ci`. Cheapest and most-often-forgotten first, then the ground
this gate newly covers, then the longest and best-known leg last — so a missing SPDX
header surfaces in seconds instead of 26 minutes in. `head-deps` is leg 1 for the same
reason: on a machine without the C toolchain, `pkg-config`, GLFW or the Vulkan loader it
prints which one is missing and **exits non-zero in about a second** rather than skipping
the module and reporting green.

**Head needs a display, and the gate insists on one.** About forty tests in `head` open a
real GLFW window and a Vulkan device, and they `t.Skipf("no window/GPU available")` when
they cannot — their packages still print `ok`. So `deps-check` refuses when there is
neither a session display nor `xvfb-run`, and `make -C head test` runs under `xvfb-run -a`
when there is no session display, which is the recipe the CI `head` job already proves
works. `deps-check` also says what it cannot prove: `pkg-config --exists vulkan` shows the
**loader** is installed, never that a **driver** initialises, so it reports the Vulkan ICD
manifest count and warns at zero; `make -C head smoke` is the check that proves the whole
native stack.

The nested modules run with `GOWORK=off`: they are not listed in `go.work`, so inside them
a plain `go test ./...` fails with *directory prefix . does not contain modules listed in
