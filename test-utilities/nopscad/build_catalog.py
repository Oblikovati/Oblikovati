...
# then emits catalog.json (machine queue for the Go test suite) and CATALOG.md
# (human-readable, sorted low->high complexity).
#
# This is step 1 of the NopSCADlib-driven kernel test & hardening program.
...
def write_markdown(path: str, entries: list):
    ...
    lines = ["# NopSCADlib Part Catalog (complexity-sorted)", "",
             ...
             "sorted low->high complexity. `golden` is filled in once the "
             "module's STL reference is rendered.", ""]
    cur = None
    for e in entries:
        if e["tier"] != cur:
            cur = e["tier"]
