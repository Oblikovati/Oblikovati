...
func TestHalfSpaceCutEqualsABoundedDifference(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"sphere/oblique cap", sph, func(t *testing.T) geom.Plane { return pl(t, 1, 1, 1, math.V3(1, 1, 1)) }, false, "the cap emits with no boundary at all: the lid carries the whole section circle, the sphere face carries nothing"},
