...
func TestHalfSpaceCutEqualsABoundedDifference(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"sphere/oblique cap", sph, func(t *testing.T) geom.Plane { return pl(t, 1, 1, 1, math.V3(1, 1, 1)) }, false, "the sphere's seam run emits a meridian arc starting at the ANTIPODE of the crossing, so the cap's loop does not close and the lid has nothing to weld to"},
