...
// TestShellContainmentClassifiesSingleShell exercises ShellContainment directly
// (commit 1261acd7 cut it over to the analytic brep.ClassifyShellPoint). Unlike
// BodyContainment, ShellContainment classifies against the region a SINGLE shell
...
func TestShellContainmentClassifiesSingleShell(t *testing.T) {
	...
	if c := ShellContainment(outer, math.P3(0, 2, 2), q, 1e-6); c != ContainOn {
		t.Errorf("outer shell, wall point = %v, want on", c)
	}
	if c := ShellContainment(outer, math.P3(10, 0, 0), q, 1e-6); c != ContainOutside {
	...
	// as inside. BodyContainment, which nets the void skin's opposite contribution,
	...
	if c := ShellContainment(outer, math.P3(2, 2, 2), q, 1e-6); c != ContainInside {
	...
	if c := ShellContainment(void, math.P3(2, 2, 2), q, 1e-6); c != ContainInside {
	...
	if c := ShellContainment(void, math.P3(0.5, 0.5, 0.5), q, 1e-6); c != ContainOutside {
