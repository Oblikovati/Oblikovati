...
// not full circles — tessellate.TessellateEdge walks the whole curve domain, so a full Circle edge would mesh the
...
func TestSpherePatchDirectMeshIsCurved(t *testing.T) {
	...
	m, ok := tessellate.SpherePatchMeshOf(nil, sphere, outer, nil, tessellate.DefaultQuality())
	if !ok {
