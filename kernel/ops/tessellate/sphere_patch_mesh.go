...
// SpherePatchMesh meshes the arc-bounded sphere patch spherePatchTrimOf recognised, in the chart that
// recognition chose. ok=false when the constrained triangulation comes back empty — the recognition is
// the caller's, so nothing here re-reads the surface or re-chooses the chart.
func SpherePatchMesh(p spherePatchTrim, outer3D []math.Point3, holes3D [][]math.Point3, q Quality) (*Mesh, bool) {
	sph, chart := p.sph, p.chart
