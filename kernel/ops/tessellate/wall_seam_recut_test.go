...
func TestHoledWallMeshesLensStraddlingTheSeam(t *testing.T) {
	t.Parallel()
	s := bandCylinder(10)
	q := tessellate.Quality{ChordTolerance: 0.005, AngleTolerance: 2 * stdmath.Pi / 180}
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			m, ok := tessellate.HoledConicWallMesh(s, outer, [][]math.Point3{lens}, q)
			if !ok {
...
func TestSplitSeamBridgedRimsRecoversBothRims(t *testing.T) {
	t.Parallel()
	s := bandCylinder(10)
	top, bot, ok := tessellate.SplitSeamBridgedRims(s, bridgedWallLoop(s, 0, 10, 96))
	if !ok {
		t.Fatal("tessellate.SplitSeamBridgedRims declined a seam-bridged wall loop")
	}
	if len(top) != 96 || len(bot) != 96 {
		t.Errorf("rims came back with %d and %d points, want 96 each (a doubled seam vertex?)", len(top), len(bot))
	}
	if v := tessellate.MeanVParam(s, top); stdmath.Abs(v-10) > 1e-9 {
		t.Errorf("top rim mean v = %v, want 10 — the rims are swapped or mixed", v)
	}
	if v := tessellate.MeanVParam(s, bot); stdmath.Abs(v) > 1e-9 {
...
func TestSplitSeamBridgedRimsDeclinesNonWrappingLoop(t *testing.T) {
	...
	if _, _, ok := tessellate.SplitSeamBridgedRims(s, quarter); ok {
		t.Error("tessellate.SplitSeamBridgedRims accepted a quarter-wall loop")
	}
}
...
func boredDiskAtAzimuth(t *testing.T, deg float64) *topo.Body {
	...
	body, ok := ops.CurvedBoolean(ops.Cut, disk, rod)
	if !ok {
...
func TestCrossingCylinderBooleanStaysExactAtScale(t *testing.T) {
	...
	for _, k := range []float64{1, 5, 20} {
		...
		body, err := ops.Boolean(ops.Cut, disk, rod)
		if err != nil {
