...
// the sphere-cap CDT path. End-to-end guard on boolean.TessellateBody.
	...
	for _, name := range []string{"filleted_box", "sphere"} {
		...
		for run := range 20 {
			...
			if !meshIdentical(first, got) {
				t.Fatalf("%s run %d: boolean.TessellateBody non-deterministic on a fixed body", name, run)
			}
		}
	}
}
...
func meshIdentical(a, b *tessellate.Mesh) bool {
