...
func zoneBodyMeshArea(t *testing.T, b *topo.Body, q tessellate.Quality) float64 {
	t.Helper()
...
func TestSphereZoneMeshesFromEitherLoopShape(t *testing.T) {
	...
	for _, c := range []struct {
		...
		area := zoneBodyMeshArea(t, c.body(), tessellate.DefaultQuality())
		want := zoneTestArea()
...
func TestSphereZoneAreaConvergesWithQuality(t *testing.T) {
	...
	coarse := zoneBodyMeshArea(t, body, tessellate.DefaultQuality())
	fine := zoneBodyMeshArea(t, body, tessellate.PropertyQuality())
	if fine <= coarse {
	...
	if rel := stdmath.Abs(fine-zoneTestArea()) / zoneTestArea(); rel > 1e-3 {
		t.Errorf("at tessellate.PropertyQuality the belt area is %.6f, want %.6f (off by %.3f%%)",
...
func seamedZone(t *testing.T, sph geom.Sphere, hi, lo geom.Circle) *topo.Body {
	...
	mid := sph.Center.TranslateBy(tessellate.MeridianPerp(sph, pHi, hi.Normal.AsVector()).Scale(math.Scalar(sph.Radius)))
...
func zoneMeshArea(m *tessellate.Mesh) float64 {
	var area float64
