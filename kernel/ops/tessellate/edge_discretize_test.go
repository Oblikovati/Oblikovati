...
func TestDiscretizeEdgeIsShared(t *testing.T) {
	...
	fwd := tessellate.DiscretizeEdge(arc, tessellate.Quality{ChordTolerance: 1e-2})
	rev := probe.ReversedPoints(tessellate.DiscretizeEdge(arc, tessellate.Quality{ChordTolerance: 1e-2}))
	if len(fwd) != len(rev) {
