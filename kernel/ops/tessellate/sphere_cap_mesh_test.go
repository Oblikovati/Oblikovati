...
// held to 2%: the kernel's own ops.DefaultQuality full sphere is ~1.6% under analytic (inscribed
...
func TestSphereCapTessellatesToAnalyticVolume(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if r := ops.Validate(body); !r.Valid || !r.Closed || !r.Manifold || !body.IsSolid() {
