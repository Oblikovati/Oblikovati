...
func curvedWeldFaces(body *topo.Body, arms []edgeFillet, w cornerWeld, sphere geom.Sphere, res Resolution) ([]filletFace, string) {
	...
	for i := range bundles {
		faces = append(faces, curvedArmTrimmedFace(bundles[i], arms[i].armSurface))
	}
...
// the generating filleted edge (ADR-0043) so the blend's edges inherit a stable topological name.
func curvedArmTrimmedFace(rails armRails, surf geom.Surface) filletFace {
	return filletFace{surface: surf, loops: []filletLoop{loopFromSegs(rails.segs)}}
}
