...
func FillFourSided(neighbours [4]*topo.Body, order int) (*topo.Body, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("heal.FillFourSided: %w", err)
	}
...
func openingEdges(neighbours [4]*topo.Body) ([4]boundaryEdge, error) {
	...
	for i, b := range neighbours {
		...
		if !ok {
			return [4]boundaryEdge{}, fmt.Errorf("heal.FillFourSided: neighbour %d has no surface face", i)
		}
		faces[i], surfs[i] = f, s
...
func chainLoop(edges [4]boundaryEdge) (c0, c1, d0, d1 boundaryEdge, err error) {
	...
	if !ok0 || !ok1 || len(rest) != 1 {
		return c0, c1, d0, d1, fmt.Errorf("heal.FillFourSided: the four edges do not form a closed loop")
	}
	c1 = rest[0]
...
func edgeCurve(s geom.BSplineSurface, e geom.Boundary) geom.BSplineCurve {
	switch e {
	case geom.UMinEdge:
		return s.BoundaryUIso(false)
	case geom.UMaxEdge:
		return s.BoundaryUIso(true)
	case geom.VMinEdge:
		return s.BoundaryVIso(false)
	default:
		return s.BoundaryVIso(true)
	}
}
