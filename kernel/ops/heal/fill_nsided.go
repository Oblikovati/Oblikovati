...
func FillNSided(neighbours []*topo.Body, order int) (*topo.Body, error) {
	if len(neighbours) < 3 {
		return nil, fmt.Errorf("heal.FillNSided: needs at least 3 bounding surfaces, have %d", len(neighbours))
	}
	if len(neighbours) == 4 {
...
func openingEdgesN(neighbours []*topo.Body) ([]boundaryEdge, error) {
	...
	for i, b := range neighbours {
		...
		if !ok {
			return nil, fmt.Errorf("heal.FillNSided: neighbour %d has no surface face", i)
		}
		faces[i], surfs[i] = f, s
...
func orderLoop(edges []boundaryEdge) ([]boundaryEdge, error) {
	...
	for len(rest) > 0 {
		...
		if !ok {
			return nil, fmt.Errorf("heal.FillNSided: the %d edges do not form a closed loop", len(edges))
		}
		loop = append(loop, orient(nxt, tail, tol))
	}
	if !loop[len(loop)-1].end().IsEqualTo(loop[0].start(), tol) {
		return nil, fmt.Errorf("heal.FillNSided: the boundary edges do not close")
	}
	return loop, nil
}
