...
func CapHoles(body *topo.Body, wallFaceKeys [][]byte) (*topo.Body, error) {
	...
	result := retopo.BuildSolidFromLoops(cappedLoops(body, removed))
...
func cappedLoops(body *topo.Body, removed map[uint64]bool) []retopo.PlanarLoop {
	...
	for _, f := range body.Faces() {
		...
		pl := retopo.PlanarLoop{Normal: f.Geometry().NormalAt(0, 0), Lineage: f.Lineage()} // keep the surviving face's identity (ADR-0043)
