...
func TestCapHolesByDiameterFillsThroughHole(t *testing.T) {
	t.Parallel()
	capped, err := heal.CapHolesByDiameter(boxWithThroughHole(t), 3)
	if err != nil {
...
func TestCapHolesExplicitSelection(t *testing.T) {
	...
	capped, err := heal.CapHoles(holey, walls)
	if err != nil {
...
func TestCapHolesByDiameterIgnoresWideOpening(t *testing.T) {
	t.Parallel()
	holey := boxWithThroughHole(t)
	out, err := heal.CapHolesByDiameter(holey, 1) // the 2×2 opening is wider than 1
	if err != nil {
		t.Fatalf("CapHolesByDiameter: %v", err)
	}
...
func TestCapHolesEmptySelection(t *testing.T) {
	t.Parallel()
	if _, err := heal.CapHoles(brepfixture.Box(math.P3(0, 0, 0), 1, 1, 1), nil); err == nil {
