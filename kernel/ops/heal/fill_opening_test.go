...
func flatNeighbour(t *testing.T, x0, x1, y0, y1 float64, flipV bool) *topo.Body {
	...
	return brepfixture.SurfaceFaceBody(t, s)
}
...
func TestFillFourSidedG0InterpolatesOpening(t *testing.T) {
	...
	out, err := heal.FillFourSided([4]*topo.Body{west, east, south, north}, 0)
	if err != nil {
...
func TestFillFourSidedErrorsOnNonNurbs(t *testing.T) {
	...
	if _, err := heal.FillFourSided([4]*topo.Body{good, box, good, good}, 0); err == nil {
...
func TestFillFourSidedPlanarNeighbours(t *testing.T) {
	...
	out, err := heal.FillFourSided(ns, 2) // G2 requested; planar ⇒ falls back to G0 per side
	if err != nil {
