...
func n7CanalWeldInputs(t *testing.T, w cornerWeld, arms []edgeFillet, res opstol.Resolution) (CornerBlendPatch, canalBoundaries, []math.Point3, float64) {
	t.Helper()
...
func assertArmHostRailsOnHost(t *testing.T, arm edgeFillet, centre math.Point3, w cornerWeld, scale float64, res opstol.Resolution, tol float64, i int) {
	t.Helper()
	set, ok := solveArmSetback(arm, centre, w.radius, scale, res)
	if !ok {
...
func assertArmFarArcIsCrossSection(t *testing.T, arm edgeFillet, centre math.Point3, w cornerWeld, scale float64, res opstol.Resolution, tol float64, i int) {
	t.Helper()
