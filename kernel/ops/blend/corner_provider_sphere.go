...
type railProvider interface {
	...
	Fits(RailLoop) bool
	Build(RailLoop, opstol.Resolution) (CornerBlendPatch, Certificate, bool)
}
...
func (analyticSphereProvider) Build(loop RailLoop, scale opstol.Resolution) (CornerBlendPatch, Certificate, bool) {
	r, ok := railRadius(loop)
	if !ok {
...
func certifySphere(sph geom.Sphere, loop RailLoop, scale opstol.Resolution) Certificate {
	weld := scale.Weld()
