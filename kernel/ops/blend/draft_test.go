...
func TestDraftFilletedBodyTapers(t *testing.T) {
	t.Parallel()
	box := brepfixture.Box(math.P3(0, 0, 0), 2, 2, 2)
	filleted, err := blend.FilletEdges(box, [][]byte{brepfixture.VerticalEdgeKey(t, box)}, 0.5)
	if err != nil {
		t.Fatalf("fillet setup: %v", err)
	}
	if brepfixture.CountCylinderFaces(filleted) == 0 {
	...
	if brepfixture.CountCylinderFaces(res) == 0 {
