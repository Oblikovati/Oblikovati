...
func filletResultFaces(body *topo.Body, fils []edgeFillet, blends map[uint64]*cornerBlend, enableObstacles, enableRunout bool) ([]filletFace, bool) {
	...
	if enableObstacles || enableRunout {
		replace, extra, handled = collectRebuildFaces(body, fils, tol.ForBody(body), maps, caps, enableObstacles, enableRunout)
	}
...
func collectRebuildFaces(body *topo.Body, fils []edgeFillet, res tol.Resolution, maps filletRebuildMaps,
...
func mergeRunoutFaces(body *topo.Body, fils []edgeFillet, res tol.Resolution, maps filletRebuildMaps,
...
func transformedBodyFaces(body *topo.Body, maps filletRebuildMaps, obReplace map[uint64]filletFace) []filletFace {
	scale := tol.ForBody(body).Size() // model scale for the subs-branch survivor-arc-carry gate (I3)
