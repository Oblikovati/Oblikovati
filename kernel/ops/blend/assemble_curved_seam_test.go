...
// validate.Validate checks — even though the welded vertex order (rec.from!=a) cannot distinguish
...
// seam. Its second use stays Reversed=false, so both uses agree and validate.Validate rejects the
