...
func (coons4Provider) Build(loop RailLoop, scale opstol.Resolution) (CornerBlendPatch, Certificate, bool) {
...
func loopRails(loop RailLoop) (c0, c1, d0, d1 geom.BSplineCurve, ok bool) {
	...
	tol := opstol.ForPoints([]math.Point3{a, b, c, d}).Weld()
...
func loopRibLen(loop RailLoop) float64 {
	...
	return opstol.ForPoints(pts).Size() * ribbonSpanFactor
}
...
func certifyCoons4Patch(fill geom.BSplineSurface, rails [4]geom.BSplineCurve, sides [4]geom.FillSide, loop RailLoop, scale opstol.Resolution) Certificate {
	return Certificate{
