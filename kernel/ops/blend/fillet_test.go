...
func TestFilletOneEdge(t *testing.T) {
	...
	res, err := blend.FilletEdges(box, [][]byte{brepfixture.VerticalEdgeKey(t, box)}, 0.5)
	if err != nil {
		t.Fatal(err)
	}
	...
	if n := brepfixture.CountCylinderFaces(res); n != 1 {
...
func TestFilletFourVerticalEdges(t *testing.T) {
	...
	if n := brepfixture.CountCylinderFaces(res); n != 4 {
...
func TestFilletCornerBlend(t *testing.T) {
	...
	if c, s := brepfixture.CountCylinderFaces(res), hasSphereFaces(res); c != 3 || s != 1 {
...
func TestFilletAllBoxEdges(t *testing.T) {
	...
	if c, s := brepfixture.CountCylinderFaces(res), hasSphereFaces(res); c != 12 || s != 8 {
...
func TestFilletTwoEdgeCornerMiters(t *testing.T) {
	...
	if c, s := brepfixture.CountCylinderFaces(res), hasSphereFaces(res); c != 2 || s != 0 {
...
func TestFilletRunOutToZero(t *testing.T) {
	t.Parallel()
	box := brepfixture.Box(math.P3(0, 0, 0), 2, 2, 2)
	res, err := blend.FilletEdgesVarying(box, []blend.EdgeFilletRadii{{Key: brepfixture.VerticalEdgeKey(t, box), R0: 0.3, R1: 0}})
	if err != nil {
		t.Fatal(err)
	}
	if r := ops.Validate(res); !r.Valid || !res.IsSolid() {
...
func TestFilletRadiusMustBePositive(t *testing.T) {
	t.Parallel()
	box := brepfixture.Box(math.P3(0, 0, 0), 2, 2, 2)
	if _, err := blend.FilletEdges(box, [][]byte{brepfixture.VerticalEdgeKey(t, box)}, 0); err == nil {
