...
func (runoutCanalProvider) Build(l RailLoop, res tol.Resolution) (CornerBlendPatch, Certificate, bool) {
...
func certifyRunoutCanalPatch(surf geom.BSplineSurface, loop RailLoop, dev float64, res tol.Resolution) Certificate {
	return Certificate{
...
func tangentHostCrease(surf geom.BSplineSurface, env BallEnvelope, res tol.Resolution) float64 {
	worst := 0.0
	u0, u1 := surf.UDomain()
...
func isoCreaseAgainstHost(surf geom.BSplineSurface, host geom.Surface, u float64, res tol.Resolution) (float64, bool) {
	v0, v1 := surf.VDomain()
	worst := 0.0
