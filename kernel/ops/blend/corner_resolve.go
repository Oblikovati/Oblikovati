...
import "oblikovati.org/kernel/ops/internal/tol"
...
// then the U4-4b exact-station canal loft (RailLoop.Stations-marked dual-host CORE panels only — the
// faithful rolling-ball skin replacing coons4's ~4.5% area miss), then the general fills (4-sided
// coons4, 3-sided tri3). analyticTorus and nFan are deferred
...
func resolveBlend(loop RailLoop, scale tol.Resolution) (CornerBlendPatch, bool) {
...
func resolveBlendWith(loop RailLoop, scale tol.Resolution, tiers []railProvider) (CornerBlendPatch, bool) {
	for _, p := range tiers {
		if !p.Fits(loop) {
			continue
		}
