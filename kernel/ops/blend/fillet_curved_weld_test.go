...
func assertRailG1Bites(t *testing.T, w cornerWeld, res opstol.Resolution) {
	t.Helper()
...
// arm welds inside-out and still passes IsSolid — so it pairs closure with validate.Validate.HolesContained AND
...
func TestFilletEdges_B3VolumeRegression(t *testing.T) {
	...
	if rep := validate.Validate(body); !rep.Valid || !rep.HolesContained {
		t.Fatalf("B3 weld invalid: Valid=%v HolesContained=%v issues=%v", rep.Valid, rep.HolesContained, rep.Issues)
	}
...
func mustHostArc(t *testing.T, host geom.Surface, tor geom.Torus, w cornerWeld, res opstol.Resolution) geom.Arc3d {
	t.Helper()
