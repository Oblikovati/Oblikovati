...
func t6ObstacleCanal(t *testing.T, of *ObstacleFeature, stations int) *obstacleCanal {
	...
	rows, ok := obstacleCanalStations(env, wall, host, feet, tol.ForSize(50).Weld())
	if !ok {
...
func t6ObstacleCanalInterior(t *testing.T, of *ObstacleFeature, n int, res tol.Resolution) (float64, float64) {
	t.Helper()
