...
// proof.
func coneCornerCertFixture(t *testing.T) (math.Point3, geom.Cone, [2]*topo.Face, tol.Resolution) {
	t.Helper()
	c := caseByName(t, "C8")
	v, faces := coneCornerExactFixture(t, c)
	co := coneOf(t, c)
...
func coneCornerQuadratic(t *testing.T, c coneCornerCase) (qa, qb, qc float64, u, d, ahat math.Vector3, p0 math.Point3, co geom.Cone, res tol.Resolution) {
	t.Helper()
	v, faces := coneCornerExactFixture(t, c)
	co = coneOf(t, c)
	planes := [2]*topo.Face{faces[1], faces[2]}
...
func TestConeRealRoots_GrazingRejects(t *testing.T) {
	t.Parallel()
	d := math.V3(1, 0, 0)
	res := tol.ForSize(1e5) // band = curvedCornerBandK·res.Weld() ≈ 4e-4
	x0 := 1 - 1e-12
...
func TestConeAlphaInBand_NearCylinderRejects(t *testing.T) {
	t.Parallel()
	res := tol.ForSize(1e6) // weld ≈ 1e-3, so band = coneAlphaBandCoef·weld/|A−p₀|
	...
	if !coneAlphaInBand(healthy, math.P3(0, 0, 0), sinH, cosH, tol.ForSize(300)) {
