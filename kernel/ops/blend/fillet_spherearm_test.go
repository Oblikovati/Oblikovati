...
func TestSphereArmSurface_Oracle(t *testing.T) {
	...
	sp := d5r150(t)
	res := tol.ForSize(300) // the sphere-body diagonal scale
	for _, c := range cases {
...
func assertSphereArm(t *testing.T, sp geom.Sphere, c sphereArmTest, res tol.Resolution) {
	t.Helper()
...
func TestSphereArmSurface_Spindle(t *testing.T) {
	...
	if _, reason := sphereArmSurface(sp, pl, n, 10, tol.ForSize(50)); reason != sphereArmSpindle {
...
func TestSphereArmSurface_Clears(t *testing.T) {
	...
	if _, reason := sphereArmSurface(sp, far, n, 10, tol.ForSize(300)); reason != sphereArmClears {
...
func TestSphereArm_ConcaveDimpleHostRejects(t *testing.T) {
	...
	if tor, reason := sphereArmSurface(sp, pl, n, 10, tol.ForBody(nil)); reason != sphereArmBuilt || !nearlyArm(tor.MajorRadius, sqrt(7500)) {
	...
	if _, reason := sphereArmFillet(e, sp, pl, sphereFace, planeFace, filletPick{edge: e, r0: 10, r1: 10}, tol.ForBody(nil)); reason != sphereArmConcaveHost {
