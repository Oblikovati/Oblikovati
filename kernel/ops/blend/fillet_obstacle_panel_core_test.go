...
func u4CoreFixture(t *testing.T) (edgeFillet, []obstacleDetection, panelSpan, opstol.Resolution) {
	t.Helper()
...
func assertLoopFits(t *testing.T, label string, ef edgeFillet, dets []obstacleDetection, span panelSpan, res opstol.Resolution) {
	t.Helper()
...
func sectionEndpoint(t *testing.T, z float64, dets []obstacleDetection, ef edgeFillet, res opstol.Resolution, end float64) math.Point3 {
	t.Helper()
...
func measureIntegratorArea(t *testing.T, loop RailLoop, res opstol.Resolution) float64 {
	t.Helper()
	patch, ok := resolveBlend(loop, res)
	if !ok {
		t.Fatalf("resolveBlend ok=false")
