...
//
//	w, ok := solveCurvedCorner(sphere, arms, opstol.ForBody(body))
//	if !ok { /* decline the weld — do-no-harm, keep the clean unwelded error */ }
func solveCurvedCorner(sphere geom.Sphere, arms []edgeFillet, res opstol.Resolution) (cornerWeld, bool) {
	if len(arms) < 3 {
...
func solveArmSetback(ef edgeFillet, c math.Point3, r, scale float64, res opstol.Resolution) (armSetback, bool) {
...
func armStation(ef edgeFillet, c math.Point3, scale float64, res opstol.Resolution) (float64, bool) {
...
func torusStation(t geom.Torus, c math.Point3, scale float64, res opstol.Resolution) (float64, bool) {
...
func cylinderStation(cyl geom.Cylinder, c math.Point3, scale float64, res opstol.Resolution) (float64, bool) {
...
func railDir(h *topo.Face, c math.Point3, r float64, res opstol.Resolution) (math.UnitVector3, bool) {
...
func hostTangentPoint(surf geom.Surface, c math.Point3, r float64, res opstol.Resolution) (math.Point3, bool) {
	switch s := surf.(type) {
	case geom.Plane:
...
func coneHostTangentPoint(co geom.Cone, c math.Point3, r float64, res opstol.Resolution) (math.Point3, bool) {
...
func sphereHostTangentPoint(sph geom.Sphere, c math.Point3, r float64, res opstol.Resolution) (math.Point3, bool) {
...
func planeTangentPoint(p geom.Plane, c math.Point3, r float64, res opstol.Resolution) (math.Point3, bool) {
	n := p.Normal()
...
func cylinderTangentPoint(cyl geom.Cylinder, c math.Point3, r float64, res opstol.Resolution) (math.Point3, bool) {
	axis := cyl.AxisDir.AsVector()
...
func distinctTangentPoints(arms []armSetback, c math.Point3, r float64, res opstol.Resolution) []math.Point3 {
	tol := res.Weld() * r
	pts := []math.Point3{}
	for _, a := range arms {
