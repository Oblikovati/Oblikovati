...
func TestRetrimCurvedHost_InnerLoopSurvives(t *testing.T) {
	...
	ff, ok := retrimCurvedHost(host, nil, nil, w, res, nil)
	if !ok {
...
func assertRetrim(t *testing.T, host *topo.Face, w cornerWeld, res opstol.Resolution, wantArea float64, arcCenter math.Point3, arcRadius float64) {
	t.Helper()
	ff, ok := retrimCurvedHost(host, nil, nil, w, res, nil)
	if !ok {
