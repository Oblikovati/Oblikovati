...
func cylindricalStripSurface(t testing.TB, r, sweep, h float64) geom.BSplineSurface {
	return brepfixture.CylindricalStripSurface(t, r, sweep, h, 1)
}
...
func cylindricalStripFace(t testing.TB, s geom.BSplineSurface, r, sweep, h float64) (face *topo.Face, rails [2]*topo.Edge) {
	body, rails := brepfixture.CylindricalStripBody(t, s, r, sweep, h)
...
// surface).
func maxTriangleArea(m *blend.Mesh) float64 {
	var max float64
...
func bunchedPanel(t *testing.T) (geom.BSplineSurface, float64) {
	t.Helper()
	s := brepfixture.CylindricalStripSurface(t, brepfixture.BunchedStripRadius, brepfixture.BunchedStripSweep, brepfixture.BunchedStripHeight, brepfixture.BunchedStripBunch)
	return s, cylindricalStripArea(brepfixture.BunchedStripRadius, brepfixture.BunchedStripSweep, brepfixture.BunchedStripHeight)
}
...
func TestNurbsPcurveMeshConvergesHighAspectPanel(t *testing.T) {
	...
	face, _ := cylindricalStripFace(t, s, brepfixture.BunchedStripRadius, brepfixture.BunchedStripSweep, brepfixture.BunchedStripHeight)
	...
	for _, tol := range []float64{0.05, 0.01, 0.001, 0.0001} {
		q := blend.Quality{ChordTolerance: tol, AngleTolerance: ops.DefaultQuality().AngleTolerance}
...
func TestStarvedRailFixShrinksOverEnclosureTriangle(t *testing.T) {
	t.Parallel()
	s, truth := bunchedPanel(t)
	face, rails := cylindricalStripFace(t, s, brepfixture.BunchedStripRadius, brepfixture.BunchedStripSweep, brepfixture.BunchedStripHeight)
...
func unfixedRailLoop(t *testing.T, f *topo.Face, rails [2]*topo.Edge, q blend.Quality) ([]math.Point2, []math.Point3) {
	t.Helper()
...
// metricCDTPatch at successively smaller boundary targets h (independent of kBoundaryCells/blend.Quality)
...
func starvedSweepLoop(t *testing.T, f *topo.Face, rails [2]*topo.Edge, q blend.Quality, target float64) ([]math.Point2, []math.Point3) {
	t.Helper()
	isRail := func(e *topo.Edge) bool { return e == rails[0] || e == rails[1] }
...
func assembleLoop(t *testing.T, f *topo.Face, pointsFor func(*topo.Edge) []math.Point3) ([]math.Point2, []math.Point3) {
	...
	if n := len(p3); n > 1 && p3[0].DistanceTo(p3[n-1]) < ops.ResolutionForPoints(p3).Weld() {
		p3 = p3[:n-1]
	}
