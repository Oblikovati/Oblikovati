...
func TestArmRunoutFeet_D5MatchesOracle(t *testing.T) {
	...
	feet, ok, reason := armRunoutFeet(ef, cap, near, near, 10, tol.ForSize(300))
	if !ok {
...
func sphereContactCircleOf(t *testing.T, sphere geom.Sphere, tor geom.Torus, res tol.Resolution) geom.Circle {
	t.Helper()
...
func capContactCircleOf(t *testing.T, pl geom.Plane, tor geom.Torus, res tol.Resolution) geom.Circle {
	t.Helper()
...
func TestBiteArcBulge_SamplesObliqueTrim(t *testing.T) {
	...
	trim, ok := intersectArmCapping(edgeFillet{armSurface: tor}, cap, feet, 10, tol.ForSize(300))
	if !ok {
