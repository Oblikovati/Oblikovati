...
func TestConcaveTorusHostArmSurfaceExternal(t *testing.T) {
	...
	arm, reject := concaveTorusHostArmSurface(host, 25, 156.7, 10, +1, +1, tol.ForSize(500))
...
func TestConcaveTorusHostArmSurfaceInternal(t *testing.T) {
	...
	arm, reject := concaveTorusHostArmSurface(host, 0, 100, 30, +1, -1, tol.ForSize(800))
	if reject != torusArmBuilt {
...
func TestConcaveTorusHostArmSurfaceRejects(t *testing.T) {
	t.Parallel()
	host := weTestTorus(t, 200, 50)
	if _, reject := concaveTorusHostArmSurface(host, 0, 250, 60, +1, -1, tol.ForSize(500)); reject != torusArmSpindle {
		t.Fatalf("r=60 ≥ a=50 internal ball: got %d, want torusArmSpindle", reject)
	}
	if _, reject := concaveTorusHostArmSurface(host, 200, 250, 10, +1, +1, tol.ForSize(500)); reject != torusArmClears {
...
func TestConcaveTorusHostTubeContact(t *testing.T) {
	...
	center, radius, ok := concaveTorusHostTubeContact(host, arm, tol.ForSize(500))
	if !ok {
