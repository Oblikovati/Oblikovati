...
func TestArmFarRunout_PerpendicularCallGraphIdentity(t *testing.T) {
	...
	h0, h1, run, ok, reason := armFarRunout(fx.ef, fx.w, fx.h0, fx.h1, onlyArmFilleted(fx.ef), tol.ForSize(200))
...
func TestIntersectArmCapping_DeclinesInvalidFeet(t *testing.T) {
	...
	if c, ok := intersectArmCapping(edgeFillet{armSurface: arm}, pl, [2]math.Point3{{}, {}}, 10, tol.ForSize(200)); ok || c != nil {
...
func TestArmFarRunout_ObliqueRegime(t *testing.T) {
	...
	_, _, run, ok, reason := armFarRunout(fx.ef, fx.w, fx.h0, fx.h1, onlyArmFilleted(fx.ef), tol.ForSize(300))
...
func meridianObliqueGap(t *testing.T, name string) float64 {
	...
	if _, _, _, obOK, _ := armFarRunout(ef, w, endSeg{from: far.Point()}, endSeg{from: far.Point()}, onlyArmFilleted(ef), tol.ForSize(300)); obOK {
