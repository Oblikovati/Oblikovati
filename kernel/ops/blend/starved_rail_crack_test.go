...
func TestStarvedRailAspectGateOff(t *testing.T) {
	...
	_ = face
	for i, e := range rails {
		pts := tessellate.DiscretizeEdge(e, ops.DefaultQuality())
		if len(pts) != 2 {
...
func TestStarvedRailNoCrackAgainstNeighbourFace(t *testing.T) {
	t.Parallel()
	const r, sweep, h = brepfixture.BunchedStripRadius, brepfixture.BunchedStripSweep, brepfixture.BunchedStripHeight
	s := brepfixture.CylindricalStripSurface(t, r, sweep, h, brepfixture.BunchedStripBunch)
	...
	for _, gq := range gateQualities() {
		...
		if meshA == nil || meshB == nil {
			t.Fatalf("%s quality: tessellate.TessellateFace returned nil (faceA=%v faceB=%v)", gq.name, meshA == nil, meshB == nil)
		}
...
func railEdgeDegrees(t *testing.T, m *blend.Mesh, rail []math.Point3) []int {
	t.Helper()
	grid := ops.ResolutionForPoints(m.Positions).Weld()
...
func meshContainsPoint(m *blend.Mesh, p math.Point3, tol float64) bool {
