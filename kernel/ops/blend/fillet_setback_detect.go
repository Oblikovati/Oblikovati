...
func detectSetbackBands(ef edgeFillet, res tol.Resolution) (setbackBands, bool) {
...
// existing otherFace(footprintEdge, host) idiom (fillet_face_transform.go) and kept intact — the wall is
...
func setbackBossFrom(im runoutImprint, ef edgeFillet, res tol.Resolution) (crossingBoss, float64, bool) {
...
// frame), so it tracks the LOCAL fillet's size even when the document-wide tol.Resolution's model
...
func nonDegenerateSetback(xSetback float64, res tol.Resolution) bool {
