...
func weldCurvedArmOrFloor(body *topo.Body, fils []edgeFillet, blends map[uint64]*cornerBlend, miters map[uint64]*cornerMiter) (*topo.Body, error) {
	b, reason := assembleCurvedArmBody(body, fils, blends, miters, tol.ForBody(body))
	if reason == "" {
		if rep := validate.Validate(b); rep.Valid && rep.HolesContained && b.IsSolid() {
			return b, nil
		}
...
func assembleCurvedArmBody(body *topo.Body, fils []edgeFillet, blends map[uint64]*cornerBlend, miters map[uint64]*cornerMiter, res tol.Resolution) (*topo.Body, string) {
...
func trihedralCornerBody(body *topo.Body, fils []edgeFillet, blends map[uint64]*cornerBlend, vid uint64, res tol.Resolution) (*topo.Body, string) {
...
func solveCurvedArmCorner(arms []edgeFillet, blends map[uint64]*cornerBlend, vid uint64, res tol.Resolution) (cornerWeld, geom.Sphere, string) {
	cb, ok := blends[vid]
	if !ok {
...
func curvedWeldFaces(body *topo.Body, arms []edgeFillet, w cornerWeld, sphere geom.Sphere, res tol.Resolution) ([]filletFace, string) {
