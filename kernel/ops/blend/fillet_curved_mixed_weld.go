...
func buildCurvedMixedArms(roles mixedRoleArms) (curvedMixedArms, bool) {
	...
	topOut, err := math.UnitVector3FromVector(probe.OutwardFaceNormalAt(topFace, probe.FaceCentroid(topFace)))
	if err != nil {
		return curvedMixedArms{}, false
	}
...
func curvedMixedCornerBody(body *topo.Body, arms []edgeFillet, res opstol.Resolution) (*topo.Body, string, bool) {
...
func buildMixedArmBundle(ef edgeFillet, arm geom.Surface, cornerArc geom.Arc3d, vPoint math.Point3, filleted map[uint64]bool, r float64, res opstol.Resolution) (mixedArmBundle, string) {
	tol := res.Weld() * r
...
func mixedArmHostRails(ef edgeFillet, arm geom.Surface, nearA, nearB, vPoint math.Point3, r float64, res opstol.Resolution) (endSeg, endSeg, string) {
...
func mixedArmFarFeet(ef edgeFillet, arm geom.Surface, vPoint math.Point3, r float64, res opstol.Resolution) (math.Point3, math.Point3, string) {
	tol := res.Weld() * r
...
func assembleCurvedMixedBody(body *topo.Body, roles mixedRoleArms, corner curvedMixedCorner, r float64, res opstol.Resolution) (*topo.Body, string) {
...
func mixedHostFaces(body *topo.Body, roles mixedRoleArms, corner curvedMixedCorner, cove, convex, planar mixedArmBundle, vPoint math.Point3, res opstol.Resolution) ([]filletFace, string) {
