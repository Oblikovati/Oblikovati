...
func TestTangentChainBoxHasNoPropagation(t *testing.T) {
	...
	keys, closed, err := blend.TangentEdgeChain(box, brepfixture.VerticalEdgeKey(t, box), blend.DefaultTangentChainAngle)
	if err != nil {
		t.Fatal(err)
	}
...
func TestTangentChainRoundedRimIsClosedLoop(t *testing.T) {
	t.Parallel()
	box := brepfixture.Box(math.P3(0, 0, 0), 2, 2, 2)
	rounded, err := blend.FilletEdges(box, brepfixture.VerticalEdgeKeys(t, box), 0.5)
	if err != nil {
