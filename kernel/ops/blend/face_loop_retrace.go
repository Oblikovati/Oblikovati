...
//	tol.Resolution weld — over a stretch of positive arc length, and (2) traverse that stretch in
...
func loopRetrace(l *topo.Loop, res tol.Resolution) (float64, bool) {
	uses := l.EdgeUses()
	worst := 0.0
	for i, ua := range uses {
...
func useRetraceLength(ua, ub *topo.EdgeUse, res tol.Resolution) float64 {
...
func lengthAboveVertexNeighbourhood(ca geom.Curve3, lo, hi float64, res tol.Resolution) float64 {
