...
func TestSolveN4CornerMatchesOracle(t *testing.T) {
	...
	res := opstol.ForPoints([]math.Point3{math.P3(0, 0, 0), math.P3(200, 200, 60)})
...
// untouched. Returns the certificate and the opstol.Resolution its thresholds are read at.
func n4TestCanalCert(t *testing.T, mutate func(*RailLoop)) (Certificate, opstol.Resolution) {
	t.Helper()
	arms, _ := classifyN4MixedArms(n4TestArms(t))
	res := opstol.ForPoints([]math.Point3{math.P3(0, 0, 0), math.P3(200, 200, 60)})
	corner, ok := solveN4Corner(arms, 5, res)
