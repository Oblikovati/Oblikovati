...
func curvedBarWithStraddlingTool(t *testing.T) (bar, tool *topo.Body) {
	...
	curved, err := blend.FilletEdges(box, [][]byte{brepfixture.VerticalEdgeKey(t, box)}, 0.5)
	if err != nil {
		t.Fatalf("fillet: %v", err)
	}
	if brepfixture.CountCylinderFaces(curved) == 0 {
...
func assertReconstructedSolid(t *testing.T, res *topo.Body, wantCyl int, wantVol float64) {
	...
	if n := brepfixture.CountCylinderFaces(res); n != wantCyl {
