...
func requireSolid(t *testing.T, label string, b *topo.Body, wantVol float64) {
	...
	if v := query.BodyGeometryProperties(b, ops.DefaultQuality()).Volume; stdmath.Abs(v-wantVol) > 1e-4 {
...
func TestBooleanIdenticalOperands(t *testing.T) {
	...
	} else if v := query.BodyGeometryProperties(res, ops.DefaultQuality()).Volume; v > 1e-6 {
...
func TestIntersectSliverOverlap(t *testing.T) {
	...
	if v := query.BodyGeometryProperties(res, ops.DefaultQuality()).Volume; v > 1e-6 {
