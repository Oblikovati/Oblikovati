...
func csgOverlap(t *testing.T) (a, b *topo.Body) {
	t.Helper()
	return brepfixture.Box(math.P3(0, 0, 0), 2, 2, 2), brepfixture.Box(math.P3(1, 0, 0), 2, 2, 2)
}
...
func TestBooleanJoinIntersecting(t *testing.T) {
	...
	if got := query.BodyGeometryProperties(res, ops.DefaultQuality()).Volume; stdmath.Abs(got-12) > 1e-6 { // 8 + 8 − 4
...
func TestBooleanCutIntersecting(t *testing.T) {
	...
	if got := query.BodyGeometryProperties(res, ops.DefaultQuality()).Volume; stdmath.Abs(got-4) > 1e-6 { // 8 − 4
...
func TestBooleanIntersectIntersecting(t *testing.T) {
	...
	if got := query.BodyGeometryProperties(res, ops.DefaultQuality()).Volume; stdmath.Abs(got-4) > 1e-6 { // the overlap slab
...
func TestBooleanCutThroughHole(t *testing.T) {
	...
	bar := brepfixture.Box(math.P3(0, 0, 0), 6, 2, 2)
	tool := brepfixture.Box(math.P3(2, -1, 0.5), 2, 4, 1)
	...
	if got := query.BodyGeometryProperties(res, ops.DefaultQuality()).Volume; stdmath.Abs(got-20) > 1e-6 {
