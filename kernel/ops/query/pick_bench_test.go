...
// hovered-plane pick runs query.RayCastFaces on every body every frame during an orbit; without the
...
func BenchmarkRayCastFacesSphere(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		if _, _, ok := query.RayCastFaces(body, origin, dir, q); !ok {
...
func TestRayCastFacesCurvedDoesNotRetessellatePerCall(t *testing.T) {
	...
	if avg > ceiling {
		t.Fatalf("query.RayCastFaces on a curved face allocates %.0f/op after warm-up (ceiling %d): the "+
...
func TestPickTessMemoMatchesFreshTessellation(t *testing.T) {
	...
	_, tCold, okCold := query.RayCastFaces(cold, origin, dir, q)

	warm := sphereBallBody(t)
	query.RayCastFaces(warm, origin, dir, q) // prime the memo
	_, tWarm, okWarm := query.RayCastFaces(warm, origin, dir, q)
