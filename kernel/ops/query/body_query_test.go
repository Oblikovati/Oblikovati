...
// TestLocateUsingPointFindsEachKind: a query.query near a corner finds the vertex,
...
func TestValidateBodyEntitiesLevels(t *testing.T) {
	...
	if ok, problems := ops.ValidateBodyEntities(clean, ops.CheckGeometry, ops.DefaultQuality()); !ok {
		t.Errorf("clean box reports problems: %+v", problems)
	}
	a := brepfixture.Tetra(1, math.V3(0, 0, 0))
	bb := brepfixture.Tetra(1, math.V3(0.2, 0.2, 0.2))
	merged := topo.MergeBodies(topo.NewLineage(topo.Tok("imp", "body", 0)), true, a, bb)
	if ok, _ := ops.ValidateBodyEntities(merged, ops.CheckTopology, ops.DefaultQuality()); !ok {
		t.Error("interpenetrating shells are still topologically valid")
	}
	ok, problems := ops.ValidateBodyEntities(merged, ops.CheckGeometry, ops.DefaultQuality())
...
func TestEntityLevelChecks(t *testing.T) {
	...
	for _, e := range b.Edges() {
		if !ops.EdgeEntityValid(e) {
			t.Fatalf("cube edge %d should be valid", e.ID())
		}
	}
	for _, f := range b.Faces() {
		if !ops.FaceEntityValid(f) {
