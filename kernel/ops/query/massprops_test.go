...
func TestTetraVolumeAndCentroid(t *testing.T) {
	t.Parallel()
	const s = 2.0
	gp := query.BodyGeometryProperties(brepfixture.Tetra(s, m.V3(0, 0, 0)), ops.DefaultQuality())
...
func TestVolumeIsTranslationInvariant(t *testing.T) {
	t.Parallel()
	atOrigin := query.BodyGeometryProperties(brepfixture.Tetra(2, m.V3(0, 0, 0)), ops.DefaultQuality())
	moved := query.BodyGeometryProperties(brepfixture.Tetra(2, m.V3(100, -50, 25)), ops.DefaultQuality())
