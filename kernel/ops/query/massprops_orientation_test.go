...
func meshVolumeAt(t *testing.T, b *topo.Body, q query.Quality) float64 {
	t.Helper()
...
func assertAnalyticVolumeIgnoresQuality(t *testing.T, sphere *topo.Body, want float64, qualities []query.Quality) {
	t.Helper()
	for i, q := range qualities {
...
func TestCoarseSphereVolumeWithinChordBound(t *testing.T) {
	...
	const chord = 0.5
	v := meshVolumeAt(t, sphere, query.Quality{ChordTolerance: chord, AngleTolerance: 40 * math.Pi / 180})
	// ops.Shell bound: an inscribed polyhedron's volume deficit is at most the surface area times the
...
func TestSolidSphereInertiaIsotropic(t *testing.T) {
	...
	q := query.Quality{ChordTolerance: 0.01, AngleTolerance: 3 * math.Pi / 180}
...
func inconsistentCubeMesh(s float64) *query.Mesh {
	mesh := &query.Mesh{}
	c := [8]m.Point3{
