...
func TestConvexHullCubeExactFaceCountAndVolume(t *testing.T) {
	t.Parallel()
	const s = 2.0
	body, err := query.ConvexHull(cubeCornersMath(s), "cube")
	if err != nil {
	...
	if v := query.BodyGeometryProperties(body, ops.DefaultQuality()).Volume; math.Abs(v-s*s*s) > 1e-9 {
...
func TestConvexHullNearCoplanarValid(t *testing.T) {
	...
	if r := ops.Validate(body); !r.Valid || !r.Closed || !r.Manifold {
		t.Errorf("near-coplanar hull invalid: %+v", r)
	}
	// Volume is essentially the cube (the apex adds a negligible sliver).
	if v := query.BodyGeometryProperties(body, ops.DefaultQuality()).Volume; math.Abs(v-8) > 1e-6 {
