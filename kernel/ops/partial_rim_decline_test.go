...
func TestPartialRimChainedCutDeclinesObservably(t *testing.T) {
	...
	cut1, err := Boolean(Cut, target, tool1)
	if err != nil {
	...
	rec := &diag.Recorder{}
	cut2, err := BooleanWithDiagnostics(Cut, cut1, tool2, rec)
	if err != nil {
	...
	if !rec.Has(CodeBooleanCSGFallback) {
		t.Error("partial-rim second cut did not record CodeBooleanCSGFallback — the decline must be observable (#1724)")
	}
}
