...
// and the model Resolution. It fails the test when the body carries no single-host obstacle there: every
// caller's premise is that it does.
func obstacleFeatureFor(t *testing.T, body *topo.Body, name string, mid math.Point3, radius float64) (edgeFillet, *ObstacleFeature, obstacleGeom, Resolution) {
	t.Helper()
...
func singleEdgeFillet(t *testing.T, body *topo.Body, name string, mid math.Point3, radius float64) (edgeFillet, Resolution) {
	t.Helper()
	edge := edgeAtMidpoint(body, mid)
	if edge == nil {
		t.Fatalf("%s: filleted edge (midpoint %v) not found", name, mid)
	}
	return resolveSingleEdgeFillet(t, body, edge, name, radius), ResolutionForBody(body)
}
