...
func BridgeBodies(bodyA, bodyB *topo.Body, orderA, orderB int) (*topo.Body, error) {
	_, sA, ok := firstNurbsFace(bodyA)
	if !ok {
		return nil, fmt.Errorf("ops.BridgeBodies: body A has no NURBS surface face")
	}
	_, sB, ok := firstNurbsFace(bodyB)
	if !ok {
		return nil, fmt.Errorf("ops.BridgeBodies: body B has no NURBS surface face")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("ops.BridgeBodies: %w", err)
	}
	return fullDomainBody(br, "bridge"), nil
}
