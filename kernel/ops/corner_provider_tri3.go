...
func tri3Rails(loop RailLoop, pole int) (c0, c1, d0, d1 geom.BSplineCurve, s0, dA, dB Side, ok bool) {
	...
	tol := ResolutionForPoints([]math.Point3{x, y, apex}).Weld()
...
func certifyTri3Patch(fill geom.BSplineSurface, rails [4]geom.BSplineCurve, sides [4]geom.FillSide, loop RailLoop, scale Resolution) Certificate {
	return Certificate{
