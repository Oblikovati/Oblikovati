...
func certifyObstaclePatch(s geom.BSplineSurface, g obstaclePatchGeom, scale Resolution) Certificate {
	return Certificate{
		Closed:      true,
		WeldsArms:   true,
...
func columnFolds(s geom.BSplineSurface, u, v0, v1 float64, scale Resolution) bool {
...
func normalsReverse(s geom.BSplineSurface, va, vb float64, na, nb math.Vector3, u float64, scale Resolution, depth int) bool {
	if na.Dot(nb) > 0 {
		return false
	}
	if depth == 0 {
