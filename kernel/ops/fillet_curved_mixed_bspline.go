...
// ARM ITSELF. OCCT fills this 4-sided loop with a rational BSplineSurface; we reproduce it with the
// general coons4 fill (corner_provider_coons4.go). The four corner points are derived from OUR arm
// surfaces (DRAWEXE-validated on N4 to ≤0.02): A(113.39,67.19,55) B(118.31,66.32,50) C(116.38,61.12,45)
// D(111.59,56.99,45):
//
...
// n4RailSamples is the sample count for an on-host contact rail's fitted BSplineCurve. A denser sampling
// hugs a curved host (the torus rail) more tightly (the cubic fit's between-sample bow shrinks with span).
const n4RailSamples = 11
...
// n4CornerPts are the four corner points plus the two terminating-arm cross-section arcs (band A→B,
// ccyl C→D). Built ONCE so every welded neighbour reads the byte-identical curve.
...
// vplane D→A), and the certified coons4 BSpline patch.
type n4Corner struct {
	pts    n4CornerPts
	railBC geom.BSplineCurve // on-torus contact rail, oriented B→C
	railDA geom.BSplineCurve // on-vplane contact rail, oriented D→A
...
// solveN4Corner derives the full N4 corner (points, arcs, rails, coons4 patch) from the classified arms,
// or ok=false when a host is missing, a station has no real intersection, or the coons4 patch fails to
// build/certify — the do-no-harm floor (the corner keeps its prior declined path).
...
func n4CornerPoints(boss geom.Cylinder, torus geom.Torus, ccylBall, bandBall geom.Cylinder, vplane, tplane geom.Plane, r float64) (n4CornerPts, bool) {
	...
	return n4CornerPts{a: a, b: b, c: c, d: d, arcAB: arcAB, arcCD: arcCD}, true
}
...
// assembleN4Corner builds the two on-host rails and the coons4 patch once the four points + two arcs are
// solved. ok=false when a rail fit or the patch build/certify fails (do-no-harm floor).
func assembleN4Corner(pts n4CornerPts, arms n4MixedArms, vplane geom.Plane, boss geom.Cylinder, res Resolution) (n4Corner, bool) {
	...
	railBC, ok1 := onSurfaceRail(torus, pts.b, pts.c)
	railDA, ok2 := onSurfaceRail(vplane, pts.d, pts.a)
	if !ok1 || !ok2 {
		return n4Corner{}, false
	}
	patch, ok := n4CornerPatch(pts, railBC, railDA, arms, vplane, res)
	if !ok {
		return n4Corner{}, false
	}
...
// onSurfaceRail fits a low-degree BSplineCurve on-host from p0 to p1: it samples the straight chord and
// projects every sample onto the surface (ClosestPointOnSurface), so the rail lies on the host by
// construction and welds by point-identity to the host retrim. ok=false on a degenerate fit.
func onSurfaceRail(surf geom.Surface, p0, p1 math.Point3) (geom.BSplineCurve, bool) {
	chord := p0.VectorTo(p1)
	pts := make([]math.Point3, n4RailSamples)
	for i := 0; i < n4RailSamples; i++ {
		t := math.Scalar(float64(i) / float64(n4RailSamples-1))
		raw := p0.TranslateBy(chord.Scale(t))
		_, _, foot := geom.ClosestPointOnSurface(surf, raw)
		pts[i] = foot
	}
...
// n4CornerPatch fills the 4-cycle A→B→C→D→A with the general coons4 provider: the two arm arcs are the
// OPPOSITE (s0,s2) pair (G1 to their arm surfaces), the two on-host rails the OPPOSITE (s1,s3) pair (G1 to
// the torus arm / the vertical plane). ok=false when the fill declines or its certificate is invalid (NoFold).
func n4CornerPatch(pts n4CornerPts, railBC, railDA geom.BSplineCurve, arms n4MixedArms, vplane geom.Plane, res Resolution) (CornerBlendPatch, bool) {
	sides := []Side{
	...
	patch, cert, ok := coons4Provider{}.Build(RailLoop{Sides: sides}, res)
	if !ok || !cert.Valid(res) {
