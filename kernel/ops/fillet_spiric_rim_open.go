...
func resolveSpiricOpenArcStations(spine spiricRimSpine, psi0, dir, span float64, res Resolution) (ellipticRimStations, geom.BSplineSurface, bool) {
...
func spiricOpenArcArmEdge(body *topo.Body, e *topo.Edge, p filletPick) (edgeFillet, bool) {
	...
	_, surf, ok := resolveSpiricOpenArcStations(spine, psi0, dir, span, ResolutionForBody(body))
	if !ok {
		return edgeFillet{}, false
	}
	faces := e.Faces()
