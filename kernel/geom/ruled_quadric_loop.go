...
func (l RuledQuadricLoop) mid() float64  { return float64((l.U0 + l.U1) / 2) }
func (l RuledQuadricLoop) half() float64 { return float64((l.U1 - l.U0) / 2) }
...
func (l RuledQuadricLoop) uAt(s float64) float64 { return l.mid() - float64(l.half()*stdmath.Cos(s)) }
...
func (l RuledQuadricLoop) TangentAt(t float64) math.Vector3 {
	...
	duds := float64(l.half() * stdmath.Sin(s))
	dvds := float64(duds*co.regularDvDu(v)) + float64(l.branchRatio(s)*co.discriminantSlope()/(4*co.a))
...
func (l RuledQuadricLoop) branchRatio(s float64) float64 {
	if d, uFold := l.nearestFold(s); d < foldParamGuard {
		...
		return stdmath.Sqrt(float64(2 * l.half() / slope))
	}
	...
	return float64(branchSign(s) * l.half() * stdmath.Sin(s) / stdmath.Sqrt(disc))
}
