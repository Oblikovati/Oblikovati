...
func (q Quadric) ValueAt(p math.Point3) float64 {
	...
	return float64(w.Dot(q.M.Apply(w))) + float64(2*float64(q.G.Dot(w))) + q.K
}
...
func (s Sphere) QuadricForm() Quadric {
	return Quadric{Anchor: s.Center, M: IdentityTensor3(), K: float64(-s.Radius * s.Radius)}
}
...
func (c Cylinder) QuadricForm() Quadric {
	return Quadric{Anchor: c.Origin, M: axialTensor3(c.AxisDir.AsVector(), 1), K: float64(-c.Radius * c.Radius)}
}
...
func (c Cone) QuadricForm() Quadric {
	...
	return Quadric{Anchor: c.Apex, M: axialTensor3(c.AxisDir.AsVector(), float64(1/(cos*cos)))}
}
...
func axialTensor3(axis math.Vector3, alpha float64) SymmetricTensor3 {
	...
	return SymmetricTensor3{
		XX: 1 - float64(alpha*x*x), YY: 1 - float64(alpha*y*y), ZZ: 1 - float64(alpha*z*z),
		XY: float64(-alpha * x * y), YZ: float64(-alpha * y * z), ZX: float64(-alpha * z * x),
	}
}
...
func (m SymmetricTensor3) Apply(v math.Vector3) math.Vector3 {
	...
	return math.V3(
		math.Scalar(float64(m.XX*x)+float64(m.XY*y)+float64(m.ZX*z)),
		math.Scalar(float64(m.XY*x)+float64(m.YY*y)+float64(m.YZ*z)),
		math.Scalar(float64(m.ZX*x)+float64(m.YZ*y)+float64(m.ZZ*z)),
	)
}
...
func (m SymmetricTensor3) Norm() float64 {
	off := float64(m.XY*m.XY) + float64(m.YZ*m.YZ) + float64(m.ZX*m.ZX)
	return stdmath.Sqrt(float64(m.XX*m.XX) + float64(m.YY*m.YY) + float64(m.ZZ*m.ZZ) + float64(2*off))
}
