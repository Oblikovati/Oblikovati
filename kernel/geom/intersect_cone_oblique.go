...
func coneObliqueConic(pl Plane, cone Cone, n, axis math.Vector3) ([]Curve3, bool) {
	...
	q := conic2D{
		a: float64(a1*a1) - c2, b: float64(2 * a1 * a2), c: float64(a2*a2) - c2,
		d: float64(2*a1*aw) - float64(2*c2*w1), e: float64(2*a2*aw) - float64(2*c2*w2), f: float64(aw*aw) - float64(c2*ww),
	}
...
func (q conic2D) curve3(o math.Point3, e1, e2, n, axis math.Vector3, apex math.Point3) ([]Curve3, bool) {
	l1, l2, u1, u2 := symmetricEig2(q.a, float64(q.b/2), q.c)
	...
	f0 := q.f + float64(0.5*(float64(q.d*s0)+float64(q.e*t0))) // the conic value at its centre: λ1·u²+λ2·v²=−f0 in principal axes
	if l1*l2 > 0 {
...
func obliqueParabola(o math.Point3, lam float64, crossEig, zeroEig [2]float64, crossDir, zeroDir math.Vector3, q conic2D) ([]Curve3, bool) {
	duP := float64(q.d*crossEig[0]) + float64(q.e*crossEig[1])
	dwP := float64(q.d*zeroEig[0]) + float64(q.e*zeroEig[1])
	if stdmath.Abs(dwP) < 1e-12 {
		return nil, false // the quadratic in u has no linear w term: parallel lines, not a parabola
	}
	uStar := float64(-duP / (2 * lam))
	wStar := float64((float64(duP*duP/(4*lam)) - q.f) / dwP)
	...
	par, err := NewParabola(vertex, axisDir, crossDir, float64(1/(4*stdmath.Abs(k))))
	if err != nil {
		return nil, false
	}
...
func (q conic2D) center() (s0, t0 float64, ok bool) {
	h := float64(q.b / 2)
	det := float64(q.a*q.c) - float64(h*h)
	if stdmath.Abs(det) < 1e-12 {
		return 0, 0, false
	}
	s0 = float64((float64(-q.d/2*q.c) - float64(h*(-q.e/2))) / det)
	t0 = float64((float64(q.a*(-q.e/2)) - float64((-q.d/2)*h)) / det)
	return s0, t0, true
}
...
func obliqueEllipse(center math.Point3, n math.Vector3, l1, l2, f0 float64, dir1, dir2 math.Vector3) ([]Curve3, bool) {
	r1, r2 := float64(-f0/l1), float64(-f0/l2)
	if r1 <= 0 || r2 <= 0 {
...
func obliqueHyperbola(center math.Point3, l1, l2, f0 float64, dir1, dir2, axis math.Vector3, apex math.Point3) ([]Curve3, bool) {
	...
	a := stdmath.Sqrt(float64(-f0 / transL))
	b := stdmath.Sqrt(float64(f0 / conjL))
...
func symmetricEig2(a, h, c float64) (l1, l2 float64, u1, u2 [2]float64) {
	tr, disc := a+c, stdmath.Hypot(a-c, float64(2*h))
	l1, l2 = float64((tr+disc)/2), float64((tr-disc)/2)
...
func eigenvector2(a, h, c, l float64) [2]float64 {
	...
	return [2]float64{float64(vx / norm), float64(vy / norm)}
}
