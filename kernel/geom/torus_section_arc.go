...
// for both: the maximal v-spans where the two roots exist, with folds at the ends. [TorusSectionArc]
// carries a branch over a full period, [TorusSectionLoop] a folded window.
...
func torusHarmonicAt(t Torus, co TorusCoForm, v float64) (torusHarmonic, bool) {
	st := co.stationOn(t, v)
	if !st.invariant {
...
func (a TorusSectionArc) Kind() CurveKind { return CurveTorusSection }

// Domain returns [0, 1].
func (a TorusSectionArc) Domain() (lo, hi float64) { return 0, 1 }

// vAt maps the curve parameter to the tube angle.
func (a TorusSectionArc) vAt(t float64) float64 { return a.V0 + float64(t*(a.V1-a.V0)) }
...
func (a TorusSectionArc) TangentAt(t float64) math.Vector3 {
	v := a.vAt(t)
