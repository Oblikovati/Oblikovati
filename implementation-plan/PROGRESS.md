...
|-----|-------|:------:|------------|-------|
| 177/178-F04 | Local face ops — Shell, Move/Offset Face, Draft (real + UI) | ✅ | `kernel/ops`, `model/feature`, `app`, `head/ui` | Built on a shared retrim primitive **`ops.rebuildWithPlanes`** (`retopo.go`): clones a solid's topology, swaps each face's plane for a caller-supplied one, re-solves every vertex at its faces' plane intersections (3×3 normal eqns). **Shell** (PR #33, `ops.Shell`): inner cavity = kept faces offset inward + removed faces left flush → `solid − cavity` (coplanar boolean opens the removed faces). **Move/Offset Face** (PR #34): `ops.MoveFaces`/`ops.OffsetFaces`; Offset Face fully UI'd, MoveFaces is the vector primitive. **Draft** (PR #35, `ops.DraftFaces`): rotates each face about its hinge (neutral plane ⟂ pull through the face's lowest vertex along pull); PullDir default +Z, round-trips. Each: real geometry + ribbon button + `head/ui` dialog + e2e (DoD). Keys don't survive the retrim (new lineage, like the boolean). F04 remaining: delete-face+heal, replace-face (topology-changing), thicken (surface body). |
