...
| M22 | Sketch3D Feature Completion (3D Parity) | 🟦 | Bring `Sketch3D` to full Inventor parity across API+kernel+UI, **including** surface-derived curves (new kernel surface-intersection machinery). 12 features (F01–F12), PBIs 230–247. **Foundation done (model+API+kernel+router, all tested):** F01 spine (`contract.Sketch3D` + wire/client + router + `Sketches3D` wired into the part + `SketchData3D` round-trip), F02 base entities (point/line/circle/arc), F03 conics (ellipse/elliptical-arc; splines TODO), F04 helical curves (new `kernel/geom.Helix3d`, 100% covered). **Pending:** F03 splines, F05 geometric constraints, F06 dimensions, F07 status, F08 edit/include, F09 profiles, F10 surface-intersection kernel, F11 surface-derived curves, F12 the whole UI layer (per the API→kernel→UI sequencing). |
