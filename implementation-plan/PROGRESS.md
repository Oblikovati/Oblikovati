...
| M21 | Sketch2D Feature Completion (2D Parity) | 🟦 | Core of all 11 features landed (PBIs 200/201/202/203/205/206/209/210/211/213/215/216/218/219/220/221 ✅): the whole 2D sketch surface — entities (lines/circles/arcs/points, ellipse, elliptical arc, splines, rectangles, polygons, slots, fillet/chamfer, offset, image), full geometric + dimensional constraints, constraint-status/DOF, move/rotate/copy/mirror, rect/circular patterns, and profiles — is exposed through `/api` (contract/wire/client, discriminated `Kind`-keyed methods) with model gaps filled, each covered by model + dogfood e2e tests; lint/vet/race green. **Remaining follow-ups:** project-geometry (208, needs B-rep reference keys), fill/text (207), fixed/offset/equation splines (204), advanced dims (214), ground/offset/align/pattern constraints (212), trim/extend/split (217), AutoDimension. UI uses the existing M06 sketch tools; per-feature `head/ui` property windows for the new ops are the remaining DoD step. |
...
| 204 | Spline families & equation curves | 🟦 | `model/sketch`, `Oblikovati.API`, `addin/router` | F03. Interpolation (fit) + control-point splines exposed via `addEntity` variant + `AddSpline` client (existing app SplineTool). **Follow-ups:** FixedSpline (immutable, needs F05 project), OffsetSpline (needs F05 offset), EquationCurve (needs expression-engine parametric `t` evaluation) — parked until F05 lands the projection/offset infra. |
...
| 207 | Fill regions & text | ⬜ | — | F04 follow-up. Annotative entities (lowest geometry value); deferred. |
| 208 | Project geometry & include | ⬜ | — | F05 follow-up. Needs B-rep edge/vertex reference-key integration (M03↔part topology); deferred. |
...
| 212 | Ground/offset/align/pattern constraints | ⬜ | — | F06 follow-up. New constraint types beyond the existing set; deferred. |
...
| 214 | Advanced dimension kinds | ⬜ | — | F07 follow-up. Offset/3-point-angle/ellipse/tangent/spline dims need new model `DimKind`s; deferred. |
...
| 217 | Trim / extend / split | ⬜ | — | F09 follow-up. Interactive curve-editing verbs; deferred. |
