...
// feature fits each and builds the network via ops.NetworkSurfaceBody. It appends a surface body.
...
func (f *NetworkFeature) Recompute(in Input) (Output, error) {
	...
	body, err := ops.NetworkSurfaceBody(f.def.UCurves, f.def.VCurves)
	if err != nil {
		return Output{}, err
	}
